...
const base = '/ownerclan/products';

// 워크스페이스: 예비상품(reserve, 기본) / 상품가공(processing). 모든 요청에 ?workspace= 주입.
let _ws = '';
export function setOwnerclanWorkspace(w: string) { _ws = w === 'processing' ? 'processing' : ''; }
function WS(): Record<string, string> { return _ws ? { workspace: _ws } : {}; }
...
export async function fetchProducts(
  ...
  const { data } = await api.get<ProductListResponse>(`${base}/`, { params: { ...params, ...WS() } });
  return data;
}

export async function fetchProductDetail(id: number): Promise<ProductDetail> {
  const { data } = await api.get<ProductDetail>(`${base}/${id}/`, { params: WS() });
  return data;
}

export async function fetchStats(): Promise<ProductStats> {
  const { data } = await api.get<ProductStats>(`${base}/stats/`, { params: WS() });
  return data;
}

export async function fetchChangedFieldCounts(): Promise<ChangedFieldCounts> {
  const { data } = await api.get<ChangedFieldCounts>(`${base}/changed-fields/`, { params: WS() });
  return data;
}
...
export async function fetchUploadTask(taskId: number): Promise<UploadTaskStatus> {
  const { data } = await api.get<UploadTaskStatus>(`${base}/upload/`, { params: { task_id: taskId, ...WS() } });
  return data;
}
...
export async function deleteAllProducts(): Promise<{ deleted: number }> {
  const { data } = await api.post(`${base}/delete-all/`, { confirm: 'DELETE_ALL' }, { params: WS() });
  return data;
}

export async function deleteProductsByIds(ids: number[]): Promise<{ deleted: number }> {
  const { data } = await api.post(`${base}/delete-ids/`, { ids }, { params: WS() });
  return data;
}

export async function dedupeByName(): Promise<{ deleted: number; higher_price_removed: number; same_price_removed: number }> {
  const { data } = await api.post(`${base}/dedupe/`, {}, { params: WS() });
  return data;
}

export async function applyElevenNames(items: { code: string; name: string }[]): Promise<{ updated: number }> {
  const { data } = await api.post(`${base}/apply-eleven-name/`, { items }, { params: WS() });
  return data;
}
...
export async function fetchDistinctValues(column: FilterableColumn): Promise<DistinctValue[]> {
  const { data } = await api.get<{ values: DistinctValue[] }>(`${base}/distinct/`, { params: { column, ...WS() } });
  return data.values;
}
