...
import { Trash2, CheckSquare, Download, Hash, ListChecks, Upload, Layers, LayoutGrid, List } from 'lucide-react';
...
export default function MyProductActionBar({
  dark, view, onViewChange,
  onExcelUpload, onDeleteAll, onDeleteSelected, onDedupe, onExcelDownload, onWCodes,
  onCodeSearch, codeSearchCount, selectedCount, busy,
}: Props) {
  ...
  return (
    <div className={`flex flex-wrap items-center gap-2 rounded-xl border ${s.card} p-2.5`}>
      ...
      <button
        ...
        title={codeSearchCount > 0 ? `${codeSearchCount}개 코드 검색중 (클릭해서 수정)` : 'W코드(원본/나의) 여러 개 한꺼번에 검색'}
      >
