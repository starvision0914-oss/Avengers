...
import { fetchMyDistinctValues, type DistinctValue, type FilterableColumn } from '../../api/myproduct';
...
export default function MyProductColumnFilterPopover({
  ...
  const allChecked = filtered.length > 0 && filtered.every(v => draft.has(v.value));
    ...
    <div ref={ref}
          ...
          filtered.map(v => {
            ...
            return (
              <label key={v.value}
                ...
                <input type="checkbox" checked={checked} onChange={() => toggleOne(v.value)}
                  className="cursor-pointer accent-blue-600" />
                <span className={`flex-1 text-[12px] truncate ${checked ? s.text1 : s.text2}`} title={v.value}>{v.value}</span>
      ...
      <div className={`flex items-center gap-2 px-3 py-2 border-t ${dark ? 'border-[#2a2b35]' : 'border-gray-200'}`}>
        <button onClick={() => { onApply([]); onClose(); }}
          className={`px-2 py-1 rounded text-[11px] ${s.text3} hover:underline`}>필터 해제</button>
        <span className={`ml-auto text-[10px] ${s.text3}`}>{fmt(draft.size)}개 선택</span>
        <button onClick={() => { onApply(Array.from(draft)); onClose(); }}
