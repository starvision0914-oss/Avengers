...
export default function TaxVatPage() {
  ...
  const [platform, setPlatform] = useState<PlatformKey>('11st');
        ...
        <div className="flex items-center gap-3 mb-4 flex-wrap">
          ...
          <div className="flex border border-[#ddd] rounded overflow-hidden text-[12px]">
            {PLATFORMS.map(p => (
              <button
                key={p.key}
                onClick={() => { setPlatform(p.key); setData(null); }}
        ...
        {data && (
          <div className="bg-white border border-[#e0e0e0] rounded p-4 mb-4">
            ...
            <div className="flex gap-6 mt-3 text-[12px]">
              <span>총 {platform === 'gmarket' || platform === 'coupang' ? '총매출' : '과세매출'} <b className="text-[#0369a1]">{formatKRW(data.grand_total)}</b></span>
