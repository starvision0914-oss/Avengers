...
import type { OwnerclanProductItem, SortColumn, SortOrder, FilterableColumn } from '../../api/ownerclan';
import { themeStyles, fmt, SALE_STATUS_LABEL, SALE_STATUS_COLOR, MARKETS } from './constants';
import OwnerclanHoverImage from './OwnerclanHoverImage';
...
function changedFieldCount(p: OwnerclanProductItem): number {
  let n = 0;
...
export default function OwnerclanProductsTable({ dark, items, onSelect, sort, order, onSort, selectedIds, onToggleId, onToggleAll, filterCol, filterVals, onOpenFilter }: Props) {
...
function MarketDots({ p }: { p: OwnerclanProductItem }) {
  return (
