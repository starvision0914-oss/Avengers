"""지마켓(옥션 제외) 2026년 광고비 월별 수집 — 엑셀 + '판매예치금 발생액'.
- 범위: 2026-01 ~ 현재월, gmarket만, 월별 순차
- 복수아이디 sellerId 드롭다운 순회, traded_at 저장
- 멱등: (seller_id, market='gmarket', use_date>=2026-01-01) 삭제 후 삽입
실행: python -u -c "import crawlers.gmkt_2026" [로그인계정...]
로그: /tmp/gmkt_2026.log
"""
...
def months_2026():
    ...
    out = []
    m = 1
    while (2026, m) <= (today.year, today.month):
        out.append((date(2026, m, 1), date(2026, m, calendar.monthrange(2026, m)[1])))
        m += 1
        if m > 12:
            break
    return out
...
def dl_month(d, sid, sdt, edt):
    ...
    files = [f for f in glob.glob(DL + '/*') if not f.endswith('.crdownload')]
    if not files:
        # 엑셀 다운로드 실패(옥션 페이지는 엑셀 고장) → 그리드 폴백.
        # 그리드는 페이지당 20행 한계라, 20행 미만(=절단 아님, 전부 보임)일 때만 신뢰.
        grows = []
        for tr in d.find_elements(By.CSS_SELECTOR, '#grid_sortingData tr'):
            ...
            try:
                ...
                continue
            grows.append((dd, ta, cls(cm), cm[:255], amt, ''))
...
def save(sid, market, rows):
    ...
    if not rows:
        return 0, 0
    GmarketCostHistory.objects.filter(seller_id=sid, market=market, use_date__gte=START).delete()
    objs, seq = [], {}
    ...
    try:
        ...
        for market, url in [('gmarket', 'https://www.esmplus.com/Member/Settle/GmktSellBalanceManagement'),
            ...
            for sid in subs:
                ...
                amt, adn = save(sid, market, allr)
                p('  [%s|%s] 광고 %d건 / %s원' % (sid, market, adn, format(amt, ',')))
        return True
    finally:
        try: d.quit()
        except Exception: pass
