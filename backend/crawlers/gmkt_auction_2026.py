"""옥션 2026년 광고비 월별 수집 — 엑셀 + '판매예치금 발생액'.
- 범위: 2026-01 ~ 현재월, gmarket만, 월별 순차
- 복수아이디 sellerId 드롭다운 순회, traded_at 저장
- 멱등: (seller_id, market='auction', use_date>=2026-01-01) 삭제 후 삽입
실행: python -u -c "import crawlers.gmkt_auction_2026" [로그인계정...]
로그: /tmp/gmkt_auc_2026.log
"""
...
DL = '/tmp/gmkt_auc_2026_xl'
AD = ('CPC', 'AI매출업', '서버비용')
START = date(2026, 1, 1)
LOG = open('/tmp/gmkt_auc_2026.log', 'a')
...
def save(sid, rows):
    if not rows:
        return 0, 0
    GmarketCostHistory.objects.filter(seller_id=sid, market='auction', use_date__gte=START).delete()
    objs, seq = [], {}
...
def collect(login_id, months):
    ...
    try:
        ...
        for sid in subs:
            ...
            amt, adn = save(sid, allr)
            p('  [%s|gmarket] 광고 %d건 / %s원' % (sid, adn, format(amt, ',')))
        return True
    finally:
        try: d.quit()
        except Exception: pass
