#!/bin/bash
# 11번가 시간별 광고비(11,15,17,18,19,20,21,22시 — crontab과 동일, 2026-09-11 주석 정정) 수집 + 계정별 CPC 증가분 텔레그램.
...
if pgrep -f "manage.py crawl_11st_cost" >/dev/null 2>&1; then
    ...
    echo "$(date '+%F %T') 정규크론 우선선점(2026-09-11)" >> "$LOG"
    /usr/bin/python3 manage.py st11_priority_preempt >> "$LOG" 2>&1
