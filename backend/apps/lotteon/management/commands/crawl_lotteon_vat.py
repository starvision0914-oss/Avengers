...
class Command(BaseCommand):
    help = '롯데온 부가세신고자료 크롤링 (정산관리 > 부가세신고자료조회) → TaxVatMonthly(platform=lotteon) 저장'

    def add_arguments(self, parser):
        parser.add_argument('--accounts', nargs='*', help='특정 계정만 (login_id)')
        parser.add_argument('--start', help='시작 YYYYMM (예: 202601)')
        parser.add_argument('--end', help='종료 YYYYMM (예: 202607)')
        parser.add_argument('--no-save', action='store_true', help='DB 저장 없이 조회만')

    def handle(self, *args, **options):
        from crawlers.lotteon_vat_crawler import run_vat_accounts
