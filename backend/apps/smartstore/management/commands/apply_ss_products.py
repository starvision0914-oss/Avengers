"""
스마트스토어 전 계정 상품명 키워드 추가 + 속성 일괄 적용 (범용)
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--account-id', type=int, required=True, help='SmartStoreAccount.id')
    ...
    def handle(self, *args, **options):
        ...
        log_file = f'/tmp/ss_apply_{account_id}.log'
        ...
        for i, p in enumerate(products):
            ...
            if pr is not None and pr.status_code == 200:
                ...
                fail_cnt += 1
                logs.append({'cno': cno, 'error': pr.status_code if pr is not None else '?', 'msg': (pr.text[:200] if pr is not None else ''), 'name': cur_name})
        ...
        with open(log_file, 'w') as f:
            json.dump({'account_id': account_id, 'ok': ok_cnt, 'fail': fail_cnt, 'skip': skip_cnt, 'name_changed': name_changed, 'attr_changed': attr_changed, 'errors': logs}, f, ensure_ascii=False)
