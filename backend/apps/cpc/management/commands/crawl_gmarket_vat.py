...
class Command(BaseCommand):
    help = '지마켓 ESM 부가세신고내역(매출) 크롤링 → TaxVatMonthly(platform=gmarket) 저장'

    def add_arguments(self, parser):
        parser.add_argument('--accounts', nargs='*', help='특정 마스터계정만 (login_id)')
        parser.add_argument('--start', help='시작 YYYYMM (예: 202601)')
        parser.add_argument('--end', help='종료 YYYYMM (예: 202606)')
        parser.add_argument('--no-save', action='store_true', help='DB 저장 없이 출력만')

    def handle(self, *args, **options):
        from crawlers.gmarket_vat_crawler import run_vat_accounts
